...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < R; i++) {
            ...
            for (int j = 0; j < C; j++) {
                ...
                //지훈이 위치 저장
                if (map[i][j] == 'J') personQueue.offerLast(new Coord(i, j));
                //불 위치 저장
                else if (map[i][j] == 'F') fireQueue.offerLast(new Coord(i, j));
            }
        }
    ...
    static int bfs() {
        while (true) {
            ...
            if (personQueue.size() == 0) return -1;
